...
- **Turnkey server (`cmd/ygo-server`)**: the bundled binary wires **no authentication** of its own. As of v1.26.0 it binds `127.0.0.1:1234` (loopback only) by default and logs a prominent `SECURITY` warning if configured (via `-addr`) to bind a non-loopback address. Expose it to a network only behind an authenticating reverse proxy — or embed `provider/websocket.Server` directly and set `AuthFunc`. Before v1.26.0 the binary bound all interfaces (`:1234`) by default; operators that relied on that must now pass `-addr :1234` (or a specific address) explicitly.
